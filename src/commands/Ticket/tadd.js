...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('tadd')
        .setDescription('Add someone to the current ticket channel.')
        .setDefaultMemberPermissions(PermissionFlagsBits.ManageMessages)
        .addUserOption((option) => option
            .setName('target').setDescription('Someone who wants to be added to the channel ticket.').setRequired(true)),
                ...
                if (interaction.channel.parentId === `${ticketdata.CategoryID}`) {
                    return interaction.reply({
                        embeds: [new EmbedBuilder()
                            // .setTitle(``)
                            .setDescription(`Successfully added ${target} to channel ticket.`)
